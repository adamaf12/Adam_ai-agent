...
export function HermesDashboard({ language, onNavigate, onRunPrompt }: HermesDashboardProps) {
  ...
  return (
    <section className="feature-page max-w-6xl mx-auto px-4 py-6" dir={isAr ? 'rtl' : 'ltr'}>
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-[var(--border)]">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[var(--accent-subtle)] text-[var(--accent)] border border-[var(--border-strong)] flex items-center gap-1">
        ...
        <button
          ...
          type="button"
          className="self-start md:self-auto px-3.5 py-2 rounded-xl bg-[var(--surface-2)] hover:bg-[var(--surface-hover)] border border-[var(--border)] text-[var(--text)] text-xs font-medium flex items-center gap-2 transition cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 my-6">
        <div className="p-4 rounded-2xl bg-[var(--surface)] border border-[var(--border)] shadow-lg">
          <div className="flex items-center justify-between text-[var(--muted)] text-xs mb-2">
            <span>{isAr ? 'إجمالي المهارات المعرفية' : 'Total Neural Skills'}</span>
            <BookOpen size={16} className="text-[var(--accent)]" />
          </div>
          <div className="text-2xl font-bold text-[var(--text)] font-mono">{stats?.totalSkills ?? skills.length}</div>
        ...
        <div className="p-4 rounded-2xl bg-[var(--surface)] border border-[var(--border)] shadow-lg">
          ...
          <div className="text-2xl font-bold text-[var(--text)] font-mono">
            {stats?.averageMasteryLevel ?? 8.6} <span className="text-sm font-normal text-[var(--muted)]">/ 10</span>
          </div>
          <div className="text-[11px] text-[var(--muted)] mt-1">
        ...
        <div className="p-4 rounded-2xl bg-[var(--surface)] border border-[var(--border)] shadow-lg">
          ...
          <div className="text-2xl font-bold text-[var(--accent)] font-mono">{stats?.totalExecutions ?? 184}</div>
          <div className="text-[11px] text-[var(--muted)] mt-1">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-6">
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer ${
                  active
                    ? 'bg-[var(--accent)] text-[var(--accent-contrast)] shadow-md shadow-[var(--accent-subtle)]'
                    : 'bg-[var(--surface)] text-[var(--muted)] hover:text-[var(--text)] hover:bg-[var(--surface-hover)] border border-[var(--border)]'
                }`}
              >
                <Icon size={13} />
        ...
        <div className="relative w-full sm:w-64">
          <Search size={14} className="absolute top-1/2 -translate-y-1/2 right-3 text-[var(--muted)]" />
          <input
            type="text"
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            placeholder={isAr ? 'ابحث عن مهارة أو كلمة مفتاحية…' : 'Search skills or keywords…'}
            className="w-full pr-9 pl-3 py-1.5 rounded-xl bg-[var(--surface)] border border-[var(--border)] text-xs text-[var(--text)] placeholder-[var(--muted)] focus:outline-none focus:border-[var(--accent)]"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredSkills.map(skill => {
          ...
          return (
            <article
              key={skill.id}
              className="p-4 rounded-2xl bg-[var(--surface)] border border-[var(--border)] hover:border-[var(--accent)] transition-all flex flex-col justify-between shadow-sm hover:shadow-xl group"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-2.5">
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                      isAutonomous
                        ? 'bg-[var(--accent-subtle)] text-[var(--accent)] border border-[var(--border-strong)]'
                ...
                <h3 className="text-base font-bold text-[var(--text)] group-hover:text-[var(--accent)] transition line-clamp-1">
                  {isAr ? skill.displayNameAr || skill.name : skill.name}
                </h3>

                <p className="text-[var(--muted)] text-xs mt-1.5 line-clamp-2 leading-relaxed">
                  {skill.description}
                </p>
                ...
                <div className="flex flex-wrap gap-1 mt-3">
                  {skill.triggers.slice(0, 4).map((t, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 rounded-md bg-[var(--surface-2)] text-[var(--muted)] text-[10px] font-mono border border-[var(--border)]"
                    >
                      #{t}
                    </span>
                  ))}
                  {skill.triggers.length > 4 && (
                    <span className="text-[10px] text-[var(--muted)] self-center">+{skill.triggers.length - 4}</span>
                  )}
                </div>
              </div>
              ...
              <div className="pt-4 mt-4 border-t border-[var(--border)] flex items-center justify-between gap-2">
                <button
                  ...
                  className="text-xs font-medium text-[var(--muted)] hover:text-[var(--text)] transition cursor-pointer"
                >
                ...
                <button
                  ...
                  className="px-3 py-1 rounded-xl bg-[var(--accent)] hover:opacity-90 text-[var(--accent-contrast)] text-xs font-semibold flex items-center gap-1.5 transition shadow-sm cursor-pointer"
                >
      ...
      {activeSkillModal && (
        ...
        >
          <div
            className="bg-[var(--surface)] border border-[var(--border-strong)] rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-4"
          ...
          >
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
              ...
              <button
                onClick={() => setActiveSkillModal(null)}
                className="text-[var(--muted)] hover:text-[var(--text)] text-lg font-bold p-1 cursor-pointer"
              >
                ✕
              </button>
            </div>

            <p className="text-[var(--text)] text-xs leading-relaxed">{activeSkillModal.description}</p>
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {activeSkillModal.proceduralSteps.map((step, idx) => (
                  <li key={idx} className="text-xs text-[var(--text)] bg-[var(--surface-2)] p-2 rounded-lg border border-[var(--border)] flex items-start gap-2">
            ...
            <div>
              ...
              <ul className="space-y-1">
                {activeSkillModal.bestPractices.map((bp, idx) => (
                  <li key={idx} className="text-xs text-[var(--muted)] flex items-center gap-1.5">
            ...
            <div className="pt-4 border-t border-[var(--border)] flex justify-end gap-2">
              <button
                ...
                className="px-4 py-2 bg-[var(--accent)] hover:opacity-90 text-[var(--accent-contrast)] rounded-xl text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
              >
