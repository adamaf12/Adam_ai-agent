...
export default function App() {
  ...
  // Desktop Sidebar State - default closed for a spacious, minimalist iOS feel
  ...
  const handleSendMessage = async (text: string, attachment?: FileAttachment, isAutoRetryAttempt: boolean = false): Promise<string | undefined> => {
    ...
    try {
      ...
      // --- 50-Second Watchdog Timer Implementation ---
      let watchdogTimer: any = null;
      let hasReceivedContent = false;
      let isWatchdogTriggered = false;

        // Watchdog triggers if no tokens/content are received within 50,000 ms (50s)
        watchdogTimer = setTimeout(() => {
          if (!hasReceivedContent && !controller.signal.aborted) {
            isWatchdogTriggered = true;
            console.warn('[Chat Stream Watchdog]: 50s elapsed with 0 tokens received. Aborting stalled stream...');
            controller.abort();
          }
        }, 50000);
      ...
      const isAbort = e?.name === 'AbortError' || e?.message?.includes('aborted') || e?.message?.includes('signal is aborted');
      const errorContent = isAbort
        ? (isArabic ? '⚠️ انقطع الاتصال مؤقتاً أثناء جلب الرد. يرجى النقر أدناه لإعادة المحاولة.' : '⚠️ Request connection timed out or was interrupted. Click below to retry.')
        : (e?.message || (isArabic
  ...
  return (
    <PermissionsGate isArabic={isArabic}>
      ...
      <div className="flex-1 flex w-full relative overflow-hidden">
        ...
        <main className="flex-1 flex flex-col justify-between max-w-4xl w-full mx-auto px-2 sm:px-4 py-1">
          <ChatArea
      ...
      {/* Connected Apps & Services Integrations Modal */}
      <IntegrationsModal
