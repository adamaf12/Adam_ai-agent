  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      ...
      >
        ...
        {isAndroid ? (
          /* Dedicated Android APK Native Session (100% immune to Google WebView origin_mismatch) */
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-emerald-950/40 border border-emerald-500/40 shadow-sm">
              <div className="flex items-center gap-2 mb-2 text-emerald-400 text-xs font-bold">
                <Smartphone size={16} />
                <span>{isAr ? 'وضع تطبيق أندرويد المستقل (APK)' : 'Android Standalone App (APK)'}</span>
              </div>
              <p className="text-[11px] text-slate-300 leading-relaxed mb-3">
                {isAr
                  ? 'تم تجهيز وتفعيل حسابك بأمان محلياً على الهاتف. تعمل جميع ميزات آدم ونظامه والذاكرة المستمرة وسجل المحادثات فوراً دون أي قيود من Google Web OAuth.'
                  : 'Your profile and session are secured locally on your device. All Adam AI capabilities, agent swarm, and persistent memory work immediately.'}
              </p>
              <button
                type="button"
                onClick={() =>
