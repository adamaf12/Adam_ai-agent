...
export const MessageItem = React.memo(
  ...
  return (
    ...
    >
      {/* Avatar */}
      <AgentAvatar isUser={isUser} className="w-8 h-8 rounded-full shrink-0" />
      ...
      <div className={`max-w-[85%] md:max-w-[78%] space-y-1.5 ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        >
          ...
          {!isUser && message.modelUsed && (
            <>
              <span>•</span>
              <span
                className="inline-flex items-center gap-1 text-[10px] bg-black/5 dark:bg-white/10 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full font-mono font-medium"
                title={isArabic ? 'النموذج الفعال' : 'Active Model'}
              >
        ...
        {/* Message Box */}
        <div
          className={`p-4 rounded-[26px] text-sm leading-relaxed shadow-2xs transition-all ${
            isUser
              ? 'bg-blue-600 text-white rounded-br-[6px]'
              : message.isError
                ? 'bg-rose-500/10 text-rose-800 dark:text-rose-300 border border-rose-500/20 rounded-tl-[6px]'
                : 'ios-glass text-slate-900 dark:text-slate-100 border border-black/5 dark:border-white/10 rounded-tl-[6px]'
          }`}
        >
