...
export interface ExecutionPlan {
  route: RequestContract['executionRoute'];
  depth: RequestContract['recommendedModelDepth'];
  steps: ExecutionStep[];
...
export function buildExecutionPlan(contract: RequestContract): ExecutionPlan {
  ...
  if (contract.executionRoute === 'web_research' || contract.needsFreshKnowledge) {
    steps.push({
      id: 'research',
      kind: 'research',
      required: true,
  ...
  if (['agent_plan', 'external_execution'].includes(contract.executionRoute) ||
      ['create', 'modify', 'debug', 'execute', 'plan', 'continue'].includes(contract.taskType)) {
    steps.push({
      id: 'plan',
      kind: 'plan',
      required: true,
      description: 'Choose the smallest reliable sequence of actions with a verification condition for each.',
      acceptance: 'Every action maps to the goal and has a verification condition.',
    });
  }

  if (contract.needsExecution || contract.executionRoute === 'agent_plan') {
    steps.push({
      id: 'execute',
      kind: 'execute',
  ...
  steps.push({
    ...
    kind: 'verify',
    required: true,
    description: 'Check the result against the original goal, constraints, and consistency.',
    acceptance: 'Result passes acceptance criteria or states what remains unverified.',
  });
  ...
  steps.push({
    ...
    kind: 'respond',
    required: true,
    description: 'Return the concise completed result and necessary verification details.',
    acceptance: 'Final answer is direct, truthful, and complete.',
  });

  return {
    route: contract.executionRoute,
    depth: contract.recommendedModelDepth,
    steps,
    acceptanceCriteria: steps.map(step => step.acceptance),
  };
}
...
export function formatExecutionPlan(plan: ExecutionPlan, language: 'ar' | 'en'): string {
  ...
  const lines = plan.steps.map((step, index) =>
    language === 'ar'
      ? `${index + 1}. [${step.kind}] ${step.description} — معيار القبول: ${step.acceptance}`
      : `${index + 1}. [${step.kind}] ${step.description} — Acceptance: ${step.acceptance}`
  );
  return `\n=== ${label} ===\n${lines.join('\n')}\n=== END PLAN ===`;
}
