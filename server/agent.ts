...
function chooseModels(prompt: string, fallback: string) { const plan = routeTask({ prompt, capabilities: inferCapabilities(prompt), maxModels: 3, preferSpeed: prompt.length < 120 }); const gemini = plan.ensemble.filter(model => model.provider === 'gemini'); return gemini.length ? gemini : [{ id: fallback, provider: 'gemini' as const } as any]; }
...
export function registerAgentRoute(app: Express, apiKey: string, model: string) {
  app.post('/api/agent', async (req: Request, res: Response) => {
    ...
    const selectedModels = chooseModels(latestPrompt, model); const ai = new GoogleGenAI({ apiKey });
    ...
    try {
      ...
      let completed = false; let emittedText = false; const outputs: string[] = [];
      for (const selectedModel of selectedModels) {
        ...
        for (let attempt = 1; attempt <= DEFAULT_RETRY_POLICY.maxAttempts && !closed && !succeeded; attempt += 1) {
          try {
            ...
            const stream = await ai.models.generateContentStream({ model: selectedModel.id, contents: messages, config: { temperature: 0.35, topP: 0.9, maxOutputTokens: Math.min(4096, DEFAULT_AGENT_BUDGET.maxResponseChars), systemInstruction: systemInstruction(language, agentName), tools: [{ googleSearch: {} }] } });
            let output = '';
            for await (const chunk of stream) { if (closed) break; const text = typeof (chunk as { text?: unknown }).text === 'string' ? (chunk as { text: string }).text : ''; if (text) output += text; }
            if (output.trim()) { outputs.push(output); emittedText = true; if (selectedModels.length === 1) res.write(JSON.stringify({ type: 'delta', text: output }) + '\n'); succeeded = true; }
          } catch (error) { if (closed || emittedText || !isRetryableError(error) || attempt >= DEFAULT_RETRY_POLICY.maxAttempts) { if (!emittedText) throw error; break; } await new Promise(resolve => setTimeout(resolve, retryDelayMs(attempt))); }
        }
      }
      if (!outputs.length && !closed) throw new Error('No model returned a usable response.');
      if (!closed && selectedModels.length > 1) { const combined = outputs.join('\n\n'); res.write(JSON.stringify({ type: 'delta', text: combined }) + '\n'); }
      completed = !closed;
      if (!closed && completed) { run = appendRunEvent(run, { phase: 'verifying', at: Date.now() }); run = appendRunEvent(run, { phase: 'responding', at: Date.now() }); run = appendRunEvent(run, { phase: 'completed', at: Date.now() }); res.write(JSON.stringify({ type: 'done', model: selectedModels.map(m => m.id) }) + '\n'); res.end(); }
      else if (closed) run = appendRunEvent(run, { phase: 'cancelled', at: Date.now() });
    } catch (error: unknown) {
